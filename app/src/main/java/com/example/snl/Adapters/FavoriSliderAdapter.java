...
public class FavoriSliderAdapter extends PagerAdapter {
    ...
    public Object instantiateItem(@NonNull ViewGroup container, final int position) {
        ...
        View view = layoutInflater.inflate(R.layout.ilandetayslider_layout,container,false);//LayoutInflater xml düzenlerinizden birinden yeni View(veya Layout) bir nesne oluşturmak için kullanılır .
        ImageView imageView = (ImageView) view.findViewById(R.id.sliderIlanDetayResim);

        Picasso.with(context).load("http://192.168.1.101:80/snldb_files/" + list.get(position).getImagepath()).resize(1050,600).into(imageView);

        imageView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (list.get(position).getAdvertisementId()!=null) {
                    Bundle bundle = new Bundle(); //Burada ilanid mizi İlan Detay Fragmentına Gönderdik Ve ilgili ilan Bilgilerini Görüntüledik.
                    bundle.putString("ilanid", list.get(position).getAdvertisementId().toString());
                    ...
                    ilanDetayFragment.setArguments(bundle);//ilan id set ettik
                    fragmentTransaction.replace(R.id.container_fragment, ilanDetayFragment);
                    fragmentTransaction.addToBackStack(null);//geriye dönüşü sağla
                    fragmentTransaction.commit();//geçişin hemen olmasını sağlar
                }
            }
        });
