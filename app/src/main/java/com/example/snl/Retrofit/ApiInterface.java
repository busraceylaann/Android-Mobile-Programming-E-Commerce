...
public interface ApiInterface {
    ...
    //Get methodu ile genellikle serverdan bilgi talebinde bulunulduğu zaman kullanılır.
    //Post metodu ile genellikle kayıt gibi işlemler yapılır. Form yapısında olan veriler server tarafına aktarılır.
    //@Field parametrenizi gizleyen ve güvenliği sağlamak için  @FormUrlEncodedistek göndermek için kullanılır


    @FormUrlEncoded //Bu belirteç bize giden formatın bir form şeklinde olacağını söylüyor. Bodysinde belirtilen değerler key:value şeklinde object formatındadır.
    @POST("loginuser.php")
    Call<LoginModel> loginuser(@Field("email") String email, //Sunucu tarafinda body icinde alinacak keyler field icerigine giriliyor
    ...
    @GET("ilanlarim.php")
    Call<List<IlanlarimModel>> ilanlarim(@Query("memberId") String memberId); // Call ile başlayan yerde url kısmındaki değişkenlerin adını verip değer ataması yapmamızı sağlıyor.
    //key=value formatında olup birden fazla veri arasına & karakteri geliyor.
